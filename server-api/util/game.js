...
function calculateMahjongSetPoints(mahjong, player) {
  return new Promise(async function (resolve, reject) {
    ...
    let isWinning = await isWinningList(mahjongList);
    if (isWinning) {
      try {
        // is 对对胡?
      ...
      console.log("This set is not winning.");
      resolve({
        points: -2,
        combination: combination,
      });
...
function isWinningList(mahjongList) {
  return new Promise(async function (resolve, reject) {
    if (mahjongList.length !== 14) resolve(false); // A valid Mahjong hand must have 14 tiles.
    ...
    resolve(true); // All sets and the pair are valid
  });
}
