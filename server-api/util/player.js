...
router.put("/", async (req, res) => {
  try {
    ...
    player.mahjong.flowerTiles.mahjongTile = player.mahjong.flowerTiles.mahjongTile.map(
      (item) => item.uid ?? item
    );
    ...
    player.mahjong.handTiles.mahjongTile = player.mahjong.handTiles.mahjongTile.map(
      (item) => item.uid ?? item
    );
  ...
  try {
    ...
    if (list.length > 0) {
      if (req.headers.pin === list[0].pin) {
        ...
        res
          .status(400)
          .json(responseModel({ isSuccess: false, responseMessage: "Incorrect Pin." }));
      }
    } else {
      res.status(200).json(
        responseModel({
