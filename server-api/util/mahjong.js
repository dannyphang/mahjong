...
router.post("/calculateFlowerTilePoints", async (req, res) => {
  try {
    res
      .status(200)
      .json(
        responseModel({ data: games.calculateFlowerTilePoints(req.body.mahjong, req.body.player) })
      );
  } catch (error) {
...
router.post("/isConsecutive", async (req, res) => {
  try {
    res
      .status(200)
      .json(
        responseModel({ data: games.isConsecutive(req.body.code1, req.body.code2, req.body.code3) })
      );
  } catch (error) {
    console.error("Error:", error);
    res.status(400).json(
