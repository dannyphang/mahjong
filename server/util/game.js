...
function calculateMahjongSetPoints(mahjong) {
  return new Promise(async function (resolve, reject) {
    let finalPoint = 0;
    let combination = {
      isDuiDuiHu: false,
    ...
    let mahjongList = mahjong.handTiles.mahjongTile.concat(mahjong.publicTiles.mahjongTile);
    ...
    if (isWinningList(mahjongList)) {
      ...
      // check if 平胡?
      if (
        combination.isQuanTongZi &&
        (checkPingHu(mahjongList) || isPingHuWithJokers(mahjongList))
      ) {
...
function checkPingHu(mahjongList) {
  ...
  function canFormPingHu(tiles, melds = 0, pairFound = false) {
    ...
    for (let i = 0; i < tiles.length; i++) {
      ...
      if (
        i + 2 < tiles.length &&
        tiles[i].code === tiles[i + 1].code &&
        tiles[i].code === tiles[i + 2].code
      ) {
        const remainingTiles = tiles.filter(
          (_, idx) => idx !== i && idx !== i + 1 && idx !== i + 2
        );
...
function isWinningList(mahjongList) {
  if (mahjongList.length !== 14) return false; // A valid hand must have exactly 14 tiles.

  const jokers = mahjongList.filter((tile) => tile.joker).length;
  const nonJokerTiles = mahjongList.filter((tile) => !tile.joker);

  // Group tiles by their code for easy counting
  const groupedTiles = nonJokerTiles.reduce((counts, tile) => {
    counts[tile.code] = (counts[tile.code] || 0) + 1;
    return counts;
  }, {});

  const counts = Object.values(groupedTiles);

  // Validate the hand to ensure 4 sets (Pungs or Chows) and 1 pair
  return validateHand(counts, jokers);
}

function validateHand(counts, jokers) {
  let sets = 0;
  let pair = 0;

  // First, check for the pair: we need exactly one pair of identical tiles
  for (let i = 0; i < counts.length; i++) {
    if (counts[i] === 2) {
      pair++;
      counts[i] = 0; // Remove the pair
    }
  }

  // If we have more than 1 pair, it's invalid
  if (pair > 1) return false;

  // Now check for sets (Pung or Chow):
  for (let i = 0; i < counts.length; i++) {
    // Handle Pungs (3 identical tiles)
    while (counts[i] >= 3) {
      sets++;
      counts[i] -= 3; // Remove the Pung
    }

    // Handle Chows (3 consecutive tiles in the same suit)
    if (i + 2 < counts.length && counts[i] > 0 && counts[i + 1] > 0 && counts[i + 2] > 0) {
      sets++;
      counts[i]--;
      counts[i + 1]--;
      counts[i + 2]--;
  ...
  // Handle jokers:
  for (let i = 0; i < counts.length; i++) {
    if (counts[i] === 2 && jokers > 0) {
      // Use 1 joker to form a pair
      sets++;
      jokers--;
    }
    if (counts[i] === 1 && jokers >= 2) {
      // Use 2 jokers to form a Pung
      sets++;
      jokers -= 2;
    }
  }

  // We need exactly 4 sets and 1 pair for a valid hand
  return sets === 4 && pair === 1;
}
