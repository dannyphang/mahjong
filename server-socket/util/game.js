      ...
      room.playerList.forEach(async (p) => {
        p.action = {
          ...
          isSelfKongable:
            ...
            (await API.isKongableFromHandSet(newMahjong, player.mahjong.handTiles.mahjongTile)).data
              .data,
          isWinnable: false,
        };
        updatePlayer(p).then((playerU) => {});
...
function actions(action, room, player, selectedMahjong) {
  return new Promise(async function (resolve, reject) {
    ...
    if (room.waiting === 2 || checkActionPriority(action, room, player, selectedMahjong)) {
      // set the selected mahjong isTaken = true
      selectedMahjong.isTaken = action === "pong" || action === "kong" || action === "self-kong";
      ...
      if (mahjongInDiscardTile) {
        mahjongInDiscardTile.isTaken =
          action === "pong" || action === "kong" || action === "self-kong";
      }
      switch (action) {
        case "pong":
          room.playerList.forEach((p) => {
      ...
      // other player also got fake set, but the current player trigger here 1st and also not next player
...
async function checkActionPriority(action, room, player, selectedMahjong) {
  ...
  switch (action) {
    case "pong":
      if (currentPlayerSameTileCount >= 2) {
        return true;
      }
      break;
  }
